...
async function check(manager_role_type, manager_role_detail_code) {
  return new Promise( async(resolve, reject) => {
    //format
    manager_role_type        = string.createcode(manager_role_type);
    manager_role_detail_code = string.createcode(manager_role_detail_code);
    // check user role
    const role = await roles.findOne({type: manager_role_type});
    
    if (!role || role.status == 0) {
      return resolve('This role is not exist or be disable in your account');
    };
    ...
    if(!role_code) {
      return resolve(`This role code is not existance`);
    }
    resolve(true);
  });
}
